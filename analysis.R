...
counties <- unite(counties,location, c(county:state), na.rm = T, sep=",", remove=F)
...
national <- national %>%
  mutate(new_cases = cases - lag(cases, n=1, default = first(cases)))
...
# HINT: The dyplr lag() function will be very helpful here.
national <- national %>%
  mutate(new_deaths = deaths - lag(deaths, n=1, default = first(deaths)))
...
lockdown_cases <- national %>%
  ...
  select(cases)
pre_lockdown_cases <- national %>%
  filter(date == "2020-02-15") %>%
  select(cases)
post_lockdown_cases <- national %>%
  filter(date == "2020-04-15") %>%
  select(cases)
pre <- lockdown_cases - pre_lockdown_cases
post <- post_lockdown_cases - lockdown_cases
my_answer <- post - pre
